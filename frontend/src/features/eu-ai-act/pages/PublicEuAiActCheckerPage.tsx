...
import { AlertCircle, CheckCircle2, ChevronLeft, ChevronRight, Scale } from 'lucide-react';
...
import type { EuAiActQuestion, EuAiActQuestionPack, EuAiActVisibilityCondition } from '../types';
...
export default function PublicEuAiActCheckerPage() {
  ...
  const progress = visibleSteps.length ? ((currentIndex + 1) / visibleSteps.length) * 100 : 0;
  ...
  const handleAnswer = async (question: EuAiActQuestion, value: unknown) => {
    ...
    if (showRequiredErrors && currentStep) {
      const stepIsComplete = currentStep.questions.every((entry) => {
        ...
        if (entry.type === 'multi') return Array.isArray(currentValue) && currentValue.length > 0;
        if (entry.type === 'boolean') return typeof currentValue === 'boolean';
        if (entry.type === 'text') return typeof currentValue === 'string' && currentValue.trim().length > 0;
