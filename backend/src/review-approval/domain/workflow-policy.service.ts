...
export class WorkflowPolicyService {
  canTransitionProject(fromStatus: ProjectWorkflowStatus, toStatus: ProjectWorkflowStatus) {
    return PROJECT_TRANSITIONS[fromStatus].includes(toStatus);
  }

  canTransitionSection(fromStatus: SectionWorkflowStatus, toStatus: SectionWorkflowStatus) {
    return SECTION_TRANSITIONS[fromStatus].includes(toStatus);
  }

  assertProjectTransition(fromStatus: ProjectWorkflowStatus, toStatus: ProjectWorkflowStatus) {
  ...
  assertSectionTransition(fromStatus: SectionWorkflowStatus, toStatus: SectionWorkflowStatus) {
  ...
  canStartReview(context: ProjectWorkflowPermissionContext) {
    return this.isAdmin(context) || context.project.reviewerId === context.actor.id;
  }
  ...
  canApproveProject(context: ProjectWorkflowPermissionContext) {
    return this.isAdmin(context) || context.project.approverId === context.actor.id;
  }
  ...
  canReviewSection(context: SectionWorkflowPermissionContext) {
    return this.isAdmin(context) || context.section.project.reviewerId === context.actor.id;
  }

  assertProjectPermission(action: ProjectPermissionAction, context: ProjectWorkflowPermissionContext) {
    const sameCompany =
  ...
  assertSectionPermission(action: SectionPermissionAction, context: SectionWorkflowPermissionContext) {
    const sameCompany =
  ...
  private isAdmin(
    context: ProjectWorkflowPermissionContext | SectionWorkflowPermissionContext,
  ) {
    return context.membership?.role === 'ADMIN' || context.actor.role === 'ADMIN';
  }
}
