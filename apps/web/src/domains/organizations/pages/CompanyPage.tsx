...
export default function CompanyPage() {
  ...
  const hasWorkspaceContext =
    ...
    (hasCompanyMembership ||
      (createdCompanyId && createdCompanyId === activeCompanyId));
  ...
  const leaveCompanyMutation = useMutation({
    ...
    onError: (err: any) => {
      toast.error(
        err?.response?.data?.message ?? 'Unable to leave the company',
      );
    },
  });
  ...
  const createCompanyMutation = useMutation({
    mutationFn: (name?: string) =>
      api
        .post('/company/create', { name: name?.trim() || undefined })
        .then((res) => res.data),
    onSuccess: (data) => {
              ...
              {company && (
                <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-4">
                  ...
                  <p className="mt-1 text-xs text-slate-500">
                    Leaving will remove your access to the shared workspace. You
                    can join a different company or create a fresh personal
                    workspace afterward.
                  </p>
