...
import {
  loadPublicQuestionPack,
  runQuickAssessment,
} from '@/domains/regulatory-frameworks/api';
import type {
  EuAiActQuestion,
  EuAiActQuestionPack,
...
const DEMO_SCENARIO = {
  ...
  inputs:
    'Credit history, declared income, employment data, repayment patterns, fraud checks.',
  outputs:
...
export default function LoanApprovalDemoPage() {
  ...
  useEffect(() => {
    ...
    const boot = async () => {
      try {
        ...
        setAnswers(
          pruneHiddenAnswers(created.questionPack, LOAN_APPROVAL_DEMO_ANSWERS),
        );
      } catch (err: any) {
        if (!active) return;
        setError(
  ...
  const handleAnswer = (question: EuAiActQuestion, value: unknown) => {
    const next = pruneHiddenAnswers(questionPack, {
      ...answers,
      [question.key]: value,
    });
    setAnswers(next);
    ...
    if (showRequiredErrors) {
      const hasMissing = visibleSteps.some((step) =>
        step.questions.some(
          (entry) =>
            entry.required && !isQuestionAnswered(entry, next[entry.key]),
        ),
      );
      if (!hasMissing) {
  ...
  const handleRunDemo = async () => {
    if (!allRequiredSatisfied) {
      ...
      setError(
        'Complete the required demo fields before running the assessment.',
      );
      return;
    }
    setSubmitting(true);
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <p className="mt-3 max-w-3xl text-sm leading-7 text-slate-600 sm:text-base">
                  This page is prefilled with a realistic consumer loan approval
                  scenario, so you can walk prospects through a live compliance
            ...
            <div className="grid gap-6 xl:grid-cols-[360px_minmax(0,1fr)]">
              <div className="space-y-6 xl:sticky xl:top-24 xl:h-fit">
                <Card className="border-slate-200/90 bg-white/95 shadow-[0_30px_80px_-56px_rgba(15,23,42,0.3)]">
                  ...
                  <CardContent className="space-y-4 text-sm text-slate-600">
                    <ScenarioRow
                      label="AI system"
                      value={DEMO_SCENARIO.systemName}
                    />
                    <ScenarioRow
                      label="Company"
                      value={DEMO_SCENARIO.company}
                    />
                    <ScenarioRow
                      label="Industry"
                      value={DEMO_SCENARIO.industry}
                    />
                    <ScenarioRow
                      label="Use case"
                      value={DEMO_SCENARIO.useCase}
                    />
                    <ScenarioRow
                      label="Input data"
                      value={DEMO_SCENARIO.inputs}
                    />
                ...
                <Card className="border-slate-200/90 bg-white/95">
                  ...
                  <CardContent className="space-y-3 text-sm text-slate-600">
                    <p>
                      This preset is designed to show a useful “action required”
                      result rather than a blank pass/fail outcome.
                    </p>
                    <ul className="space-y-2 text-slate-700">
                      <li>
                        High-risk context: access to credit and essential
                        services
                      </li>
                      <li>
                        Strong oversight present, but documentation gaps remain
                      </li>
                      <li>
                        Missing conformity readiness creates a clear remediation
                        story
                      </li>
                    </ul>
              ...
              <div className="space-y-6">
                ...
                {visibleSteps.map((step, index) => (
                  <Card
                    key={step.key}
                    className="border-slate-200/90 bg-white/95"
                  >
                    <CardHeader>
                ...
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <div className="text-sm text-slate-500">
                    Use the preset as-is or change answers live to show
