...
@Injectable()
export class ReopenProjectAfterSectionEditUseCase {
  constructor(
  ...
  async execute(params: { projectId: string; actorId: string; note?: string }) {
    const context = await this.context.loadProjectContext(
      params.projectId,
      params.actorId,
    );
    this.policy.assertProjectSystemTransition(
    ...
    await this.projects.transitionProject({
      ...
      expectedVersion: context.project.workflowVersion,
      note: params.note ?? 'Section edited; re-review required',
    });
