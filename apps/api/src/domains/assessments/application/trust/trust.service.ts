...
export class TrustService {
  ...
  async analyzeFairness(
    ...
    for (const r of rows) {
      ...
      const yPred = hasPred
        ? Number(r[dto.columns?.y_pred || 'y_pred'])
        : undefined;
      if (!groups.has(g))
  ...
  async analyzeFairnessSegments(
    ...
    for (const seg of dto.segments) {
      ...
      const rows = allRows.filter(
        (r) => String(r[column]) && set.has(String(r[column])),
      );
      if (!rows.length) {
      ...
      const hasPred =
        hasPredGlobal || (rows.length && yPredCol && yPredCol in rows[0]);
      for (const r of rows) {
        const g = String(r[sensitiveCol]);
        const yTrue = Number(r[yTrueCol]);
      ...
      const rates = Array.from(groups.values()).map((g) =>
        g.total > 0 ? g.positive / g.total : 0,
      );
      const maxRate = rates.length ? Math.max(...rates) : 0;
      const minRate = rates.length ? Math.min(...rates) : 0;
  ...
  async analyzeDrift(
    ...
    const candidateCols =
      dto.columns && dto.columns.length ? dto.columns : Object.keys(cRows[0]);
    ...
    const psiAvg = psiVals.length
      ...
      : 0;
    const klAvg = klVals.length
      ? klVals.reduce((a, b) => a + b, 0) / klVals.length
      : 0;
