...
import type { LoginCommand, SignupCommand } from './auth.commands';
...
export class AuthService {
  ...
  async signup(dto: SignupCommand) {
    ...
    let assignedRole: Role | undefined =
      createdCompany || isPowerAdmin ? 'ADMIN' : undefined;
    ...
    const adminUser = isPowerAdmin
      ? await this.ensurePowerAdmin(user as any)
      : user;
  ...
  private async resolveCompany(dto: SignupCommand): Promise<{
    companyId: string;
