...
import type {
  OrganizationActor,
  OrganizationRole,
} from '../../domain/membership/organization-membership.types';
...
export class InvitationsService {
  ...
  private async resolveRoleForNewMember(
    companyId: string,
    requestedRole: OrganizationRole = 'USER',
  ): Promise<'USER' | 'REVIEWER' | 'ADMIN'> {
    ...
    return requestedRole === 'COMPANY_ADMIN' ? 'ADMIN' : requestedRole;
  }

  private ensureAdmin(user: OrganizationActor, companyId: string) {
  ...
  async listInvitations(user: OrganizationActor, companyId: string) {
    this.ensureAdmin(user, companyId);
